...
let curPointCloudScanMesh = [];
let curPointCloudScanNoGroundMesh = [];
...
let parameters = {
    ...
    datasets: labelTool.currentDataset,
    sequences: labelTool.currentSequence,
...
function loadPCDData() {
    ...
    pcdLoader.load(pointCloudFullURL, function (mesh) {
      ...
      curPointCloudScanMesh = mesh;
      if(labelTool.filterGround == false){
        scene.add(mesh);
      }
    });
    ...
    pcdLoader.load(pointCloudWithoutGroundURL, function (mesh) {
      ...
      curPointCloudScanNoGroundMesh = mesh;
...
function calculateProjectedBoundingBoxAdvan(xPos, yPos, zPos, width, length, height, channel, rotationY) {
    ...
    if (ev.button === 0) {
        ...
        if (clickedObjects.length > 0 && clickedObjectIndex !== -1) {
            ...
            if (clickedObjectIndexPrevious !== -1) {
                console.log("clickedObjectIndexPrevious: ", clickedObjectIndexPrevious);
...
function init() {
    ...
    if (guiBoundingBoxAnnotationMap === undefined) {
        ...
        chooseSequenceDropDown = guiOptions.add(parameters, 'sequences', 
            labelTool.sequencesNuScenes).name("Choose Sequence")
            .onChange(function (value) {
        ...
        filterGroundCheckbox.onChange(function (value) {
            ...
            if (labelTool.filterGround === true) {
                ...
                addObject(curPointCloudScanNoGroundMesh, "pointcloud-scan-no-ground-" + labelTool.currentFileIndex);
            } else {
                ...
                addObject(curPointCloudScanMesh, "pointcloud-scan-" + labelTool.currentFileIndex);
            }
        });
