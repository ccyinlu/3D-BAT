...
function annotationFileExist(fileIndex, channel) {
    ...
    if (labelTool.showOriginalNuScenesLabels === true) {
        ...
        url = 'input/' + labelTool.currentDataset + '/' + labelTool.currentSequence + '/annotations/LIDAR_TOP/' + labelTool.currentDataset + "_" + labelTool.currentSequence + '_annotations.txt';

    }

    // console.log("annotationFileExist: ", url);
...
function parseAnnotationFilename(fileName) {
  let rawFile = new XMLHttpRequest();
  let annotationsFilenameArray = [];

  let annotationFilenameFullURL = 'input/' + labelTool.currentDataset +'/'+labelTool.currentSequence + '/' + fileName;
  // console.log(annotationFilenameFullURL);

  try {
    rawFile.open("GET", annotationFilenameFullURL, false);
  } catch (error) {
  }
