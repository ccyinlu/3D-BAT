...
if ~exist('data_root')
  data_root = '/media/bingo/SSD/annotation/roadSide/demo';
end
...
params_config_file = [data_root '/' 'points_config.yml'];
...
% add the support for ouster lidar, position -> [ring_num x horizon_num x 3], intensity -> [ring_num x horizon_num]
if length(size(pointCloudBase.Location)) == 3
  H = size(pointCloudBase.Location, 1);
  W = size(pointCloudBase.Location, 2);
  points = [reshape(pointCloudBase.Location, [H*W 3]) reshape(pointCloudBase.Intensity, [H*W 1])];
else
